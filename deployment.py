...
loaded_model = pickle.load(open('D:/Degree/Year 3/Trimester1_2210/Data Mining/Stream/trained_model.sav', 'rb'))
...
option = st.sidebar.selectbox("Choose the technique or feature: ", ['Feature Selection','Basket Size Predictive Model','Clustering', 'Email'])
...
if option == "Clustering":
    ...
    Spectacles_No = st.text_input("Did the customer wear spectacles? (0 for Yes and 1 for No)")
    if Spectacles_No == "":
    ...
    ShirtType_LongSleeve = st.text_input("Did the customer wear long sleeve shirt? (0 for No and 1 for Yes)")
    ...
    Attire_Traditional = st.text_input("Did the customer wear traditional attire? (0 for No and 1 for Yes)")
    ...
    Attire_Casual = st.text_input("Did the customer wear casual attire? (0 for No and 1 for Yes)")
    if Attire_Casual == "":
        st.error("This field is required.")
    else:
        Attire_Casual = int(Attire_Casual)
        if not (Attire_Casual == 0 or Attire_Casual == 1):
            st.error("Invalid input, please enter either 0 or 1")

    Kids_Category_No_Kids = st.text_input("Did the customer had kids? (0 for Others and 1 for No Kids)")
    ...
    With_Kids_Yes = st.text_input("Did the customer comes with kids? (1 for Yes and 0 for No)")
    if With_Kids_Yes == "":
        st.error("This field is required.")
    else:
    ...
    With_Kids_No = st.text_input("Did the customer comes with kids? (0 for Yes and 1 for No)")
    if With_Kids_No == "":
        st.error("This field is required.")
    else:
        With_Kids_No = int(With_Kids_No)
        if not (With_Kids_No == 0 or With_Kids_No == 1):
            st.error("Invalid input, please enter either 0 or 1")
